...
export async function fetchAplicacoes(params?: {
  ...
  const url = `${API_BASE_URL}/aplicacoes${searchParams.toString() ? `?${searchParams}` : ""}`
  const response = await fetch(url)
  if (!response.ok) throw new Error("Failed to fetch applications")
  return response.json()
}

export async function fetchAplicacaoPorNome(nome: string): Promise<Aplicacao | null> {
...
export async function fetchDependencias(aplicacaoId: number): Promise<any[]> {
  const response = await fetch(`${API_BASE_URL}/aplicacoes/${aplicacaoId}/dependencias`)
  if (!response.ok) return []
  return response.json()
}

export async function fetchVulnerabilidades(aplicacaoId: number): Promise<any[]> {
  const response = await fetch(`${API_BASE_URL}/aplicacoes/${aplicacaoId}/vulnerabilidades`)
  if (!response.ok) return []
  return response.json()
}
